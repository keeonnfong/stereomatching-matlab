...
for kz = FirstPlane:numel(zPlanes)
    ...
    for kcam = FirstCam:Ncam
        filename = sprintf('%s/CalibrationPlan_%02d_cam%d.%s',dirIn, kz, kcam, extension);        
...
%         aRoi = [1,1;1268,1;1268,720;1,720]; % this is for MoraPTVdata
%         aRoi = [1,1;800,1;800,800;1,800]; % this is for CAPSULE 2cam
%         [pimg,pos3D,T3px2rw,T3rw2px,T1rw2px,aRoi]=calib2D(Img,th,dotSize,gridSpace,lnoise,kz,z,xyzRef(kcam).ref(kz,:),kcam,dirIn,aRoi);
        if size(th,2)>1
            [pimg,pos3D,T3px2rw,T3rw2px,T1rw2px,aRoi]=calib2D(Img,th(kcam),dotSize,gridSpace,lnoise,kz,z,xyzRef(kcam).ref(kz,:),kcam,dirIn);
        else
